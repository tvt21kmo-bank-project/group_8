...
nosto::nosto(QWidget *parent, int tilinro, bool luotolle, int korttiNro) :
    QDialog(parent),
    ui(new Ui::nosto)
{
    ui->setupUi(this);
...
void nosto::setupUI(QString tila)          // Muotoillaan formin layouttia kulloisen toiminallisuuden vaiheen mukaiseksi
{
    if(tila == "alkuarvot")
    {
        muuSummaValikko = 0;
...
void nosto::tarkistaKate(int valittuSumma)     // Haetaan tietokannasta tilin tiedot katteen riittävyyden tarkastusta varten
{
    setupUI("loppuEvent");
...
void nosto::onkoKatetta(QNetworkReply *reply)  // Tarkastetaan riittääkö tilillä kate käyttäjän pyytämään nostotapahtumaan
{
    double saldo = 0;
...
void nosto::timerLogout()          // mikäli käyttäjä jumittaa 10s painamatta mitään nappia suljetaan nostotapahtuma ja palaan menuun
{
    ui->btnOtaRahat->hide();
    ui->labelMuuSumma->show();
...
void nosto::on_btn40_clicked()     // Napeilla suoritetaan nostettavan rahasumman valinta
{
    objTimer->start(10000);
...
void nosto::on_btnOtaRahat_clicked()               // Ota rahat napilla lähetetäåän tietokantaan pyyntö kirjata kyseinen nostotransaktio
{
...
void nosto::onnistuikoNosto(QNetworkReply *reply)      // Tarkastetaan responsetiedoista onnistuiko kirjaaminen tietokantaan. Mikäli ei niin revitään käyttäjältä rahat kourasta takaisin masiinaan ja kerrotaan, että vituiks meni
{
    int onnistuiko;
    QByteArray response_data = reply->readAll();
    QJsonDocument json_doc = QJsonDocument::fromJson(response_data);
