...
INSERT INTO `pankkitili` VALUES (3,12452,-3601,14811),(4,5806,NULL,NULL),(5,8128.85,-311,2000);
...
START TRANSACTION;
	...
	SET test2=ROW_COUNT();
    IF (test2 > 0) THEN SET onnistuiko = 1;
    ELSE SET onnistuiko = 0;
    END IF;
    SELECT onnistuiko;
END ;;
...
BEGIN
  ...
  IF luotto = 1 THEN
	UPDATE pankkitili SET creditsaldo=creditsaldo-summa WHERE tilinumero=tilinro;
ELSE
	UPDATE pankkitili SET creditsaldo=creditsaldo-summa WHERE tilinumero=tilinro;
END IF;
  SET test1=ROW_COUNT();
    IF (test1 > 0) THEN   
      COMMIT; 
      INSERT INTO tapahtumat(idtapahtumat, aikaleima, tapahtuma, summa, credit, tilinumero, maksaja_saaja) VALUES(NULL, NOW(), 'Nosto tililtä', -summa, 0, tilinro, CONCAT("Pankkikortti#",id1));
    ELSE
      ROLLBACK;
  END IF;
  END ;;
...
BEGIN
  ...
  SET test1=ROW_COUNT();
    IF (test1 > 0) THEN   
      COMMIT;    
      INSERT INTO tapahtumat(idtapahtumat, aikaleima, tapahtuma, summa, credit, tilinumero, maksaja_saaja) VALUES(NULL, NOW(), 'Talletus', summa, 0, tilinro, CONCAT("Pankkikortti#",id1));
    ELSE
      ROLLBACK;
  END IF;
  END ;;
...
BEGIN
	SELECT * FROM tapahtumat WHERE tilinumero=tilinumero1 AND credit=luotto ORDER BY aikaleima DESC LIMIT alkaenRivilta, 10;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2021-12-15 16:13:01
